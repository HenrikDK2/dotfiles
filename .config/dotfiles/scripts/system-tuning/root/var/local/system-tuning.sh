...
# Disable/Enable pcscd depending on if a smart card reader is detected
if service_exists "pcscd.service"; then
    if ls /dev/bus/usb/*/* 2>/dev/null | xargs -I{} udevadm info --query=property --name={} 2>/dev/null | grep -q "ID_SMARTCARD_READER=1"; then
...
if is_laptop; then
    ...
    # Enable switcheroo-control on laptops
    if service_exists "switcheroo-control.service"; then
    ...
    # Disable switcheroo-control on desktops (only useful for hybrid GPU laptops)
    if service_exists "switcheroo-control.service"; then
